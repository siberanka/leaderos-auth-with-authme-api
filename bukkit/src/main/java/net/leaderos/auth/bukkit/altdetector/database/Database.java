...
package net.leaderos.auth.bukkit.altdetector.database;
...
public abstract class Database
{
    ...
    String countAccountsByIp = "SELECT COUNT(DISTINCT playerid) AS account_count FROM {prefix}iptable WHERE ipaddr = ? AND date >= datetime('now', ?);";
    ...
    public Database(AltDetectorController plugin, boolean debug, String prefix)
    {
        this.plugin = plugin;
        this.debug = debug;
        this.prefix = prefix;
    ...
    public int countAccountsByIp(String ip, int expirationTime)
    {
        int count = 0;

        try (Connection conn = getConnection();
             PreparedStatement stmt = conn.prepareStatement(replacePrefix(countAccountsByIp)))
        {
            stmt.setString(1, ip);
        ...
        {
            plugin.getLogger().warning("Database error counting accounts for IP " + ip + ": " + e.getMessage());
        }
